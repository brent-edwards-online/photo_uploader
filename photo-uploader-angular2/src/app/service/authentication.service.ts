...
@Injectable() export class AuthenticationService {
     ...
     */
    public signinSubject = new BehaviorSubject<boolean>(this.tokenNotExpired()); // Initial value
    ...
    public login(username: string, password: string): Observable<any> {
        ...
        return this.http.post(baseURL + tokenEndpoint, body, this.options)
            .map((res: Response) => {
                let body: any = res.json();
    ...
    public revokeRefreshToken(): void {
        ...
        if (refreshToken != null) {
            ...
            this.http.post(baseURL + revocationEndpoint, body, this.options)
                .subscribe(
                () => {
    ...
    private tokenNotExpired(): boolean {
        ...
        let result = token != null && (Helpers.getExp() > new Date().valueOf()); 
        return result;
    }
